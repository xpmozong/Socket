...
- (void)commonInit
{
    _labelText              = [[NSMutableString alloc] init];
...
- (M80AttributedLabelURL *)linkDataForPoint:(CGPoint)point
{
...
#pragma mark - 添加链接
- (void)addCustomLink: (NSURL *)linkData
...
{
    M80AttributedLabelURL *linkData = [self linkDataForPoint:point];
    if (linkData)
    {
...
- (void)addAutoDetectedLink: (M80AttributedLabelURL *)link
{
    ...
    [self addCustomLink:link.linkInfo forRange:link.range linkType:link.linkType];
}
...
- (void)touchesBegan:(NSSet *)touches withEvent:(UIEvent *)event
{
    NSLog(@"touchesBegan");
    if (_fromType == 1) {
        [_delegate m80AttributedLabelClick];
        return;
    }
